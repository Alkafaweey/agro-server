...
from .forms import profileForm, SkillForm, ExperienceForm
...
def profiles(request):
    profiles, search_content = searchProfiles(request)

    custom_range, profiles = paginateProfiles(request, profiles, 3)

    context = {'profiles':profiles, 'search_content':search_content, 'custom_range':custom_range}
...
def userProfile(request, pk):
    ...
    context = {'profile':profile, 'topSkills':topSkills, 'otherSkills':otherSkills, 'experience':experience}
...
def userAccount(request):
    ...
    context = {'profile':profile, 'skill':Skills, 'projects':projects, 'experience':experience}
...
def AddExperience(request):
    ...
    return render(request, 'users/experience-form.html', context)
